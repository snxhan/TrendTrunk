...
<body>
    ...
    <div class="main">
        <div class="main-shrink">
            <h1 class="homepage_header">Change Password Outcome</h1>
            <div class="row">
                <div class="col-2">
                    <?php
                    // Variables from account_change_password.php form
                    ...
                    if (isset($_SESSION['logged_on_user_email'])) {
                        ...
                        $stmt->bind_param('s', $_SESSION['logged_on_user_email']);
                        $stmt->execute();
                        ...
                        if ($num_results > 0) {
                            $retrievedPassword = $mdarray[0]['password'];
                        }
                        // hash current password
                        $hashedcurrentPassword = md5($currentPassword);
                        // Check if same
                        if ($retrievedPassword == $hashedcurrentPassword) {
                            // Update accordingly  
                            // Prepare Query
                            $query = "UPDATE users SET password= MD5(?) WHERE email = ?";
                            // Querying the Database
                            $stmt = $db->prepare($query);
                            $stmt->bind_param('ss', $newPassword, $_SESSION['logged_on_user_email']);
                            $stmt->execute();
